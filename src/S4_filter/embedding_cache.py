...
class EmbeddingCache:
    """SQLite-based embedding cache with model-aware invalidation."""
...
class CachedEmbedder:
    """Embedder wrapper with caching support and cross-model priority lookup."""
    ...
    def __init__(self, inner: Embedder, cache: EmbeddingCache, embedding_dim: int | None = None):
        ...
        self._stats = {"hits": 0, "misses": 0, "reused_from_other_model": 0}
    ...
    def encode(self, texts: list[str]) -> np.ndarray:
        """
        Encode texts with caching and cross-model priority lookup.

        Cache lookup uses priority: api > prod > dev.
        Only reuses cache entries with matching dimensions.
        ...
        # 1. Lookup cache (with dimension filter for cross-model compatibility)
        cached = self.cache.get_many(texts, target_dim=self._embedding_dim)

        # Track if we reused from other models
        ...
        # 3. If all cached, return directly
        if not missing_indices:
            return np.stack([cached[i] for i in range(n)])

        # 4. Compute missing embeddings
        ...
        # 6. Merge results in original order
        ...
        if len(missing_indices) > 0:
            logger.info(
                f"Embedding cache: {len(cached)} hits, {len(missing_indices)} misses "
                f"({len(cached)/(len(cached)+len(missing_indices))*100:.1f}% hit rate), "
                f"model={self.cache.model_id}"
            )
